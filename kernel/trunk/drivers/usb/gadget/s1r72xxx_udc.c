...
void * s1r72xxx_ep_alloc_buffer(struct usb_ep *_ep, unsigned int bytes,
	dma_addr_t * dma, unsigned int gfp_flags);
static void s1r72xxx_ep_free_buffer(struct usb_ep *_ep, void *buf,
...
static int s1r72xxx_usbc_suspend(struct device *dev, pm_message_t state,
	u32 level);
static int s1r72xxx_usbc_resume(struct device *dev, u32 level);
int usb_gadget_register_driver(struct usb_gadget_driver *driver);
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver);
static irqreturn_t s1r72xxx_usbc_irq(int irq, void *_dev, struct pt_regs *r);
...
static int proc_udc_show(struct seq_file *s , void * a)
{
	...
	if (toggle == 0){
		s1r72xxx_usbc_suspend(dev_temp->dev, dummy_pm, 1);
		toggle = 1;
		seq_printf(s, "suspend \n");
	} else {
		s1r72xxx_usbc_resume(dev_temp->dev, 1);
		toggle = 0;
		seq_printf(s, "resume \n");
	}
#else
...
static struct usb_ep_ops s1r72xxx_ep_ops = {
	...
	.alloc_buffer	= s1r72xxx_ep_alloc_buffer,
	.free_buffer	= s1r72xxx_ep_free_buffer,
...
/* ========================================================================= */
/**
 * @brief	- allocate an I/O buffer
 * @par		usage:
 *				API(see functional specifications: 8.1.5.)
 * @param	*_ep;		endpoint structure that specifies the endpoint.
 * @param	bytes;		length of buffer.
 * @param	dma;		pointer to allocated memory, must be phyisical address.
 * @param	gfp_flags;	flags to allocate memory.
 * @retval	return pointer to allocated memory.
 *			in case of error, then return NULL.
 */
/* ========================================================================= */
void * s1r72xxx_ep_alloc_buffer(struct usb_ep *_ep, unsigned int bytes,
	dma_addr_t * dma, unsigned int gfp_flags)
{
	...
	/**
	 * - 2. Allocate memory:
	 *  - call kmalloc. where size is 'bytes'.
	 *  - set pointer to 'dma' to physical address where allocated above.
	 *  - in case of error or DMA isn't supported, return NULL.
	 */
	req = kmalloc(bytes, gfp_flags);
	if (req == NULL)
	{
		DEBUG_MSG("%s, kmalloc failed\n", __FUNCTION__);
		return req;
 ...
 * @par		usage:
 *				API(see functional specifications: 8.1.6.)
 * @param	*_ep;		endpoint structure that specifies the endpoint.
...
static int __init s1r72xxx_usbc_probe(struct device *dev)
{
	 ...
	 */
	usbc_dev = kmalloc(sizeof(S1R72XXX_USBC_DEV), SLAB_KERNEL);
	if ( usbc_dev == NULL ) {
	 ...
	 */
	res = platform_get_resource_byname(pdev,IORESOURCE_MEM, S1R72_DEVICE_NAME);
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_IRQ,
		S1R72_DEVICE_NAME);
	retval = request_irq(res->start, s1r72xxx_usbc_irq, SA_SHIRQ,
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	 ...
	 */
	if ( (driver == NULL) 
		|| (driver->bind == NULL) || (driver->unbind == NULL)
