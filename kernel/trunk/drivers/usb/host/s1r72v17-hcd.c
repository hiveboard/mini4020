/**
 * @file	s1r72v17-hcd.c
 * @brief	S1R72V17 HCD (Host Controller Driver) for USB
 * @date	2007/04/17
...
static const char	hcd_name[] = "S1R72V17-hcd";
#define	DRV_NAME	"s1r72v17"

#undef	CONFIG_USB_V05_HCD
#undef	CONFIG_USB_V05_HCD_MODULE

#define	HC_INIT	v17_hc_init
#define	HC_EXIT	v17_hc_exit


#include "s1r72v17-regs.h"
...
void init_ch_info(struct hcd_priv *hcd)
{
	...
	for (ch_ct = CH_0; ch_ct < CH_MAX ; ch_ct ++){
		...
		hcd->ch_info[ch_ct].CHxJoin		= rcC_AREA0_Join_0 + S_R_CH_JOIN_OFFSET(ch_ct);
	}
...
void init_lsi(void)
{
	...
	*(volatile unsigned long *)EMI_CSDCONF_V = 0x0BFFFFF1; 

	DPRINT(DBG_INIT,"%s:enter\n", __FUNCTION__);

	printk("In %s Chip Revision No is 0x%x (0x%x)\n",DRV_NAME,RevisionNum, S_R_REGS8(rcC_RevisionNum));
	/**
	...
	if (S_R_REGS8(rcC_RevisionNum) != RevisionNum) {
		...
		S_R_REGS8(rc_ChipConfig - 1) = CPU_Endian | BE_MODE;
		/* set clock */
		S_R_REGS8(rcC_ClkSelect - 1) = CLKFreq_12M;
		/* dummy read */
		chgendian = S_R_REGS8(rcC_CPU_ChgEndian);
		/* protect mode clear */
		S_R_REGS8(rcC_ModeProtect) = ModeProtectSet;
	...
	S_R_REGS8(rc_Reset) = S_R_REG_CLEAR;
...
void init_fifo(void)
{
	 ...
	 */
	for (ch_ct = CH_0 ; ch_ct < CH_MAX ; ch_ct ++) {
		S_R_REGS8(rcC_AREA0_Join_1 + S_R_CH_JOIN_OFFSET(ch_ct)) = (1 << ch_ct);
	}
...
module_init(v17_hc_init);
module_exit(v17_hc_exit);

MODULE_AUTHOR("Luxun9 Project Team.");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("SEIKO EPSON S1R72V17 USB Host Control Driver");
