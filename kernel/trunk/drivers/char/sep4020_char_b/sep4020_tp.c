...
#define CSL     *(volatile unsigned long*)GPIO_PORTD_DATA_V &= ~(0x1<<3) //cs 片选信号拉低
#define CSH     *(volatile unsigned long*)GPIO_PORTD_DATA_V |= 0x1<<3		//cs 片选信号拉高

#define CLKL    *(volatile unsigned long*)GPIO_PORTD_DATA_V &= ~(0x1<<4)	//时钟输入口线
#define CLKH    *(volatile unsigned long*)GPIO_PORTD_DATA_V |= 0x1<<4

#define DATAL   *(volatile unsigned long*)GPIO_PORTD_DATA_V &= ~(0x1<<1)	//命令输入口线
#define DATAH   *(volatile unsigned long*)GPIO_PORTD_DATA_V |= 0x1<<1
 ...
 static unsigned short PenSPIXfer(unsigned short ADCommd)
{
    ...
    CLKL;
    udelay(10);			
    if(*(volatile unsigned long*)GPIO_PORTD_DATA_V & 0x1)	//判断数据输出位是否为1
        {
        data |= 0x1; 
		}	
    data <<= 1;		
...
static void sep4020_tp_setup(void)
{
    ...
    *(volatile unsigned long*)GPIO_PORTD_DIR_V 	|= 0x01;		//portD0 输入
    *(volatile unsigned long*)GPIO_PORTD_DIR_V 	&= ~(0x1a);	//prortD1, portD3, portD4输出
    *(volatile unsigned long*)GPIO_PORTD_SEL_V	|= 0x1b;   	//0，1，3，4均设置为通用（普通io）
