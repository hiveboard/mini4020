       	...
       	hdc = BeginPaint (hWnd);
        	...
        	for(i=0;i<BTM_NUM;i++)
          	...
          	if(xmButton[i].mode == BTM_UP)
           	 	Draw3DUpFrame (hdc, xmButton[i].x,  xmButton[i].y ,xmButton[i].x+xmButton[i].w,xmButton[i].y+xmButton[i].h,PIXEL_lightwhite);
          	else  
					if(xmButton[i].mode == BTM_DOWN)
            		Draw3DDownFrame (hdc, xmButton[i].x,  xmButton[i].y ,xmButton[i].x+xmButton[i].w,xmButton[i].y+xmButton[i].h,PIXEL_lightwhite);
         			FillBoxWithBitmap (hdc, xmButton[i].x+xmButton[i].bmpx,xmButton[i].y+xmButton[i].bmpy,0, 0, &xmButton[i].bitmap);
...
static void InitControlPanelInfo (PMAINWINCREATE pCreateInfo)
{
    ...
    pCreateInfo->spCaption = "USB HOST";
    pCreateInfo->hMenu = 0;
...
void initBTMs(char *dirpath)
{
   ...
   closedir(dir);
	...
	XMBUTTON xmButtonTemp,xmButtonTemp1;
  	...
  	for(i=0;i< (k + l);i++)
	{
  	...
  	for(i=0;i< (k + l);i++)
    	{  		
		printf("i =%d\n",i);
      xmButton[i].x= 10+(i%5)*55;//170 +
      xmButton[i].y=20+((int)(i/5))*80;//20 + 
      xmButton[i].h=60;
      xmButton[i].w=50;
      xmButton[i].bmpx=5;
      xmButton[i].bmpy=2;
      ...
      xmButton[i].titlex=(8-j)*3;
      xmButton[i].titley=42;
