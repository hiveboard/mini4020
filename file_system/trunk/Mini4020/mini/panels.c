#include "mini.h"
static char staticpath[20] = {0};
static DLGTEMPLATE DlgJPG =
{
    WS_BORDER | WS_CAPTION,WS_EX_NONE,0, 0, 320, 240,"Picture Show",0, 0, 1, NULL, 0
};
...
int SDOpenPanel(HWND hWnd,int id,int type,char* path)
{
	printf("type = %d\n",type);

	printf("path = %s\n",path);
	bzero(staticpath,20);
	strcat(strcpy(staticpath,path),"/");
	printf("staticpath = %s\n",staticpath);
  	switch(type)
  	{
        case  IDM_JPG:
