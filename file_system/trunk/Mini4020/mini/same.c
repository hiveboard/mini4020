...
#define STONE_COLS        8
#define STONE_LINES        4

//#define ID_TIMER        1900
...
static void check_game_over (HWND hwnd)
{
    ...
    if (cleared){
        ...
        end_of_game (hwnd, _("The Same Game"));
    }
    else
        end_of_game(hwnd, _("The Same Game"));
}
...
static HMENU createpmenusettings (void)
{
    ...
    mii.type        = MFT_STRING;
    mii.id          = 0;
    mii.typedata    = (DWORD)"Settings";
    hmnu = CreatePopupMenu (&mii);
    
    mii.type        = MFT_STRING ;
    mii.state       = 0;
    mii.id          = IDM_PREF;
    mii.typedata    = (DWORD)"Preferences...";
    InsertMenuItem(hmnu, 0, TRUE, &mii);
...
static HMENU createpmenuabout (void)
{
    ...
    mii.state       = 0;
    mii.id          = IDM_ABOUT;
    mii.typedata    = (DWORD)"About Same Game...";
    InsertMenuItem(hmnu, 0, TRUE, &mii);
...
int SameGameProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    case MSG_CREATE:
    {
        ...
        CreateWindow (CTRL_STATIC, "Scenario: ", 
        ...
        hwnd_score = CreateWindow (CTRL_STATIC, "Score:", 
                    WS_CHILD | WS_VISIBLE | SS_RIGHT, 0,
...
static void InitCreateInfo (PMAINWINCREATE pCreateInfo)
{
    ...
    pCreateInfo->by += GetSysCharHeight () + 12;
    pCreateInfo->iBkColor = PIXEL_black;
    pCreateInfo->dwAddData = 0;
   // pCreateInfo->hHosting = HWND_DESKTOP;
}

int Same_Game(HWND hWnd)
{
    HWND hMainWnd;
    MSG Msg;
